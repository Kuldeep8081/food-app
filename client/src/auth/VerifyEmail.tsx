...
const VerifyEmail = () => {
  ...
  const handleChangeDown=(index:number,e:React.KeyboardEvent<HTMLInputElement>)=>{
    ...
    <div className="flex items-center justify-center h-screen w-full">
        <div className="p-8 rounded-md w-full max-w-md flex flex-col gap-10 border border-gray-200">
              ...
              <form action="">
                <div className="flex justify-between">
                  {
                    otp.map((letter:string,idx:number)=>(
                      <Input 
                      key={idx}
                      ref={(el) => void (inputRef.current[idx] = el)}
                      type="text"
                      maxLength={1}
                      value={letter}
                      onChange={(e:ChangeEvent<HTMLInputElement>)=>handleChange(idx,e.target.value)}
                      onKeyDown={(e:React.KeyboardEvent<HTMLInputElement>)=>handleChangeDown(idx, e)}
                ...
                {
                  loading? <Button disabled className=" mt-6 w-full"><Loader2 className="mr-2 w-4 h-4 animate-spin" /> Please Wait</Button>
